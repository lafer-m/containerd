...
// LoopDevice describes a loop device
...
func (loop *LoopDevice) AttachFromFile(image *os.File, mode int, number *int) error {
	var path string
	var loopFd int
		...
		*number = device
		...
		if loop.Shared {
			...
			if err != nil {
				...
				logrus.Debugf("Could not get loop device %d status: %s", device, err)
				continue
			}
			...
			if status.Inode == imageIno && status.Device == imageDev &&
				...
				logrus.Debugf("Sharing loop device %d", device)
				loop.fd = new(int)
				*loop.fd = loopFd
				return nil
			}
...
func (loop *LoopDevice) AttachFromPath(image string, mode int, number *int) error {
...
func (loop *LoopDevice) Close() error {
	if loop.fd != nil {
